...
@param Dir where to find file of queries:
@param File with last gen of queries:
...
file_url = SAVE_DIR + fileName_queries.replace("_queries_last_gen.txt",
                                               "_EVALUATION_")
...
def eval_and_calculate_poblational_fitness(invalid_ind, pop, num_q, gen_number):
    ...
    for i in invalid_ind + pop:
        ...
        if (str(OBJ_COMBINATION) == '1'):                    
            ...
            i.fitness.values = (i.precision, i.jaccard_index_indiv, 
                                len(i.retrieved_that_are_relevant))                                    
...
def main():    
    ...
    ''' Llamo con Pop = [] ya que pop == invalid_ind'''
    ...
    """ ********ANTES ERA POR CADA GENERACION, AHORA SOLO LA ULTIMA******** """
    ...
    print "-END OF EVALUATION (last gen)-"
