...
for run in range(0,nRuns):
    ...
    if (combination in ['1']):                
        ...
        plt.scatter(Xs_precision, Ys_recall, c='#007FAE', marker = 'o', linewidths = 1, facecolor = '#007FAE', label= 'Co1')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#007FAE')
    elif (combination in ['2']):
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#00BDBD', marker = 'd', linewidths = 1, facecolor = '#00BDBD', label= 'Co2')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#00BDBD')
    elif (combination in ['3']):
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#FF6600', marker = '<', linewidths = 1, facecolor = '#FF6600', label= 'Co3')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#FF6600')
    elif (combination in ['4']):
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#8BAD00', marker = 'p', linewidths = 1, facecolor = '#8BAD00', label= 'Co4')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#8BAD00')            
    elif (combination in ['5']):            
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#7F3300', marker = 's', linewidths = 1, facecolor = '#7F3300', label= 'Co5')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#7F3300' )
    elif (combination in ['6']):            
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='green', marker = 'x', linewidths = 1, facecolor = 'green', label= 'Co6')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='green' )                    
    elif (combination in ['7']):            
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#ff6347', marker =  r'$\clubsuit$', linewidths = 1, facecolor = '#ff6347', label= 'Co7')
        # Then plot the Pareto frontier on top
