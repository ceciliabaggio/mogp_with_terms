...
last_gen_precision = lines[-popSize:] 
#la cantidad de corridas es la cant de columnas
nRuns = len(last_gen_precision[1])  
...
for run in range(0,nRuns):
    ...
    filename_save_prec_rec_points = open(directoryTopic + base + 
                                             "points_Pr-Rec_last_gen_run_" +
                                              str(run+1) +".txt", "w")
    filename_save_prec_rec_pareto = open(directoryTopic + base + 
                                             "pareto_Pr-Rec_last_gen_run_" +
                                              str(run+1) +".txt", "w")
        
    Xs_precision = [fila[run] for fila in last_gen_precision]
        
    #print 'precision = ', Xs_precision
    
    Ys_recall = [fila[run] for fila in last_gen_recall]
    ...
    if (combination in ['1']):                
        ...
        plt.scatter(Xs_precision, Ys_recall, c='#007FAE', marker = 'o', s=65, linewidths = 1, facecolor = '#007FAE', label= 'Co1')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#007FAE')
    elif (combination in ['2']):
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#00BDBD', marker = 'd', s=65, linewidths = 1, facecolor = '#00BDBD', label= 'Co2')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#00BDBD')
    elif (combination in ['3']):
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#FF6600', marker = '<', s=65, linewidths = 1, facecolor = '#FF6600', label= 'Co3')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#FF6600')
    elif (combination in ['4']):
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#8BAD00', marker = 's', s=65, linewidths = 1, facecolor = '#8BAD00', label= 'Co4')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#8BAD00')            
    elif (combination in ['5']):            
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#7F3300', marker = r'$\clubsuit$', s=65,linewidths = 1, facecolor = '#7F3300', label= 'Co5')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='#7F3300' )
    elif (combination in ['6']):            
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='green', marker = 'X', s=65, linewidths = 1, facecolor = 'green', label= 'Co6')
        # Then plot the Pareto frontier on top
        plt.plot(p_front_Xs, p_front_Ys, c='green' )                    
    elif (combination in ['7']):            
        # Plot a scatter graph of all results
        plt.scatter(Xs_precision, Ys_recall, c='#ff6347', marker = '*', s=65, linewidths = 1, facecolor = '#ff6347', label= 'Co7')
        # Then plot the Pareto frontier on top
    ...
    fig_directory = directoryTopic + base + 'pareto_Pr-Rec_last_gen_run_'+ str(run+1)+'.eps'
