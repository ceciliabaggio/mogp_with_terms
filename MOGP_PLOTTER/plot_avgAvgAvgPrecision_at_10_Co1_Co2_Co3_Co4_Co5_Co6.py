...
def plot_avgAvgAvgPrecision_at_10(directory, combination):
    ...
    if (combination == 1):                
        plt.scatter(x[0::5], list_of_mean[0::5], c='#007FAE', marker = 'o', linewidth = 0.7, label= 'Co1', alpha=alph)
        l1 = plt.plot(x, list_of_mean, c='#007FAE', linewidth = 0.7) 
    elif (combination == 2):                
        plt.scatter(x[0::5], list_of_mean[0::5], c='#00BDBD', marker = 'd',  linewidth = 0.7, label= 'Co2', alpha=alph)
        l2 = plt.plot(x, list_of_mean, c='#00BDBD', linewidth = 0.7)
    elif (combination == 3):
        plt.scatter(x[0::5], list_of_mean[0::5], c='#FF6600', marker = '<',  linewidth = 0.7, label= 'Co3', alpha=alph)
        l3 = plt.plot(x, list_of_mean, c='#FF6600', linewidth = 0.7)
    elif (combination == 4):
        plt.scatter(x[0::5], list_of_mean[0::5], c='#8BAD00', marker = 's',  linewidth = 0.7, label= 'Co4', alpha=alph)
        l4 = plt.plot(x, list_of_mean, c='#8BAD00', linewidth = 0.7)           
    elif (combination == 5):            
        plt.scatter(x[0::5], list_of_mean[0::5], c='#7F3300', marker =  r'$\clubsuit$' ,  linewidth = 0.7, label= 'Co5', alpha=alph)        
        l5 = plt.plot(x, list_of_mean, c='#7F3300', linewidth = 0.7)      
    elif (combination == 6):            
        plt.scatter(x[0::5], list_of_mean[0::5], c='green', marker = 'X' ,  linewidth = 0.7, label= 'Co6', alpha=alph)
        l6 = plt.plot(x, list_of_mean, c='green', linewidth = 0.7)                
    elif (combination == 7):            
        plt.scatter(x[0::5], list_of_mean[0::5], c='tomato', marker = '*' ,  linewidth = 0.7, label= 'Co7', alpha=alph)    
        l7 = plt.plot(x, list_of_mean, c='tomato', linewidth = 0.7)
                  
    #errorbar(x(1:5:end), y(1:5:end), Array_std(1:5:end), ':bo', 'LineWidth',1,'MarkerEdgeColor','b', 'MarkerSize',7 );     

    # Hacer combinaciones en la misma fig! 
...
if len(sys.argv) == 9:
    ...
    plt.savefig(save_dir +"evolution_precision@10-concepts.svg", bbox_extra_artists=(lgd,), bbox_inches='tight')
