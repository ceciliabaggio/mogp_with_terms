...
def NORMFIT_CI_1st_and_last_gen_GLOBAL_JACCARD(directory):
      
    # Get a list of all folders in this folder --> TOPICS        
    files_in_dir= os.listdir(directory)
    files_in_dir.sort()
    
    if ('Co1_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory + 
                    'Co1_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt', 
                    unpack=False)
    ...
    if ('Co2_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory + 
                    'Co2_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt', 
                    unpack=False)
        mean = np.mean(globalRecall_by_topic)            
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co3_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory + 
                    'Co3_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co4_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory + 
                    'Co4_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co5_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory + 
                    'Co5_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co6_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory + 
                    'Co6_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co7_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory+ 
                    'Co7_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_GEN_1.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co1_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory + 
                    'Co1_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt', 
                    unpack=False)
        print (globalRecall_by_topic)        
        mean = np.mean(globalRecall_by_topic)               
        """ Usando desvio estandar -->  FORMULA DE ANA """        
    ...
    if ('Co2_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory+ 
                    'Co2_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt', 
                    unpack=False)
        mean = np.mean(globalRecall_by_topic)            
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co3_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory+ 
                    'Co3_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co4_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory+ 
                    'Co4_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co5_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory+ 
                    'Co5_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co6_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory+ 
                    'Co6_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
    ...
    if ('Co7_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt' in files_in_dir):
        globalRecall_by_topic = loadtxt(directory+ 
                    'Co7_MEAN_JACCARD_INDEX_FROM_TESTING_ALL_TOPICS_LAST_GEN.txt', 
                    unpack=False)         
        mean = np.mean(globalRecall_by_topic)        
        desvio = np.std(globalRecall_by_topic)
        print (globalRecall_by_topic)
        print (desvio)
        error =  desvio * 1.96 / math.sqrt(len(globalRecall_by_topic))     
        co7_last = plt.errorbar(7 , mean, yerr=error,  c='#ff6347',
                    marker = '*', linewidth = 1, alpha=1, capsize=4, markersize=15)
    """ PARA PRUEBA """
...
if len(sys.argv) == 2:        
    ...
    plt.savefig(save_dir+ "TESTING_CI_Global_Jaccard-terms.svg")
    #plt.show()
                  
else:
    print ("debe ingresar los directorios de donde leer. Debe contener los archivos")
    print ("del tipo Co1_MEAN_GLOBAL_JACCARD_FROM_TESTING_ALL_TOPICS_LAST_GEN")
    print (" ")
