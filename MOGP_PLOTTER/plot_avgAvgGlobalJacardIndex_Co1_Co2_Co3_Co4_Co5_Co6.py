...
def plot_avgAvgJaccardSimilarityIndex(directory, combination):
    ...
    for topic in topic_folders:
        ...
        if (topic_number == 0): 
            ...
            avgAvgAvgPrecision_Gen_x_Topic = np.zeros((rows, len(topic_folders)))
    ...
    if (combination == 1):                
        ...
        plt.scatter(x, list_of_mean, c='#00BDBD', marker = 'd',  linewidth = 0.7, label= 'Co2', alpha=0.7)
        plt.plot(x, list_of_mean, c='#00BDBD', linewidth = 0.7)          
    elif (combination == 3):
        plt.scatter(x, list_of_mean, c='#FF6600', marker = '<',  linewidth = 0.7, label= 'Co3', alpha=0.7)
        plt.plot(x, list_of_mean, c='#FF6600', linewidth = 0.7)
    elif (combination == 4):
        plt.scatter(x, list_of_mean, c='#8BAD00', marker = 's',  linewidth = 0.7, label= 'Co4', alpha=0.7)
        plt.plot(x, list_of_mean, c='#8BAD00', linewidth = 0.7)                
    elif (combination == 5):            
        plt.scatter(x, list_of_mean, c='#7F3300', marker =  r'$\clubsuit$',  linewidth = 0.7, label= 'Co5', alpha=0.7)        
        plt.plot(x, list_of_mean, c='#7F3300', linewidth = 0.7)
    elif (combination == 6):            
        plt.scatter(x, list_of_mean, c='green', marker = 'X' ,  linewidth = 0.7, label= 'Co6', alpha=0.7)    
        plt.plot(x, list_of_mean, c='green', linewidth = 0.7)         
    elif (combination == 7):            
        plt.scatter(x, list_of_mean, c='tomato', marker = '*' ,  linewidth = 0.7, label= 'Co7', alpha=0.7)    
