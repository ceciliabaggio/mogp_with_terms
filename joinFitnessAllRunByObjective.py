...
'''de prueba'''
#path = str('/home/cecilia/repos/borrar2/Co1/528')
...
for filename in files:
    ...
    elif (len(m) == 11 and m[10]=="JaccardIndex.txt"):
        indiv_jaccard_files.append(filename)  
    elif (len(m)==12 and m[9]=="height"):
        height_files.append(filename)
...
height_files = sorted(height_files , key=lambda x: int(x.split('_')[1]))
...
#===============================================================================
### PARA PRECISION
name = path +"/"+TOPIC +"_"+str(nRuns)+"_"+ objectives +"_"+ nGen+"_"+ popSize+ "_"+ indSize +"_" + cross +"_" + mut +"_precision_all_run.txt"
file_allPrecision = open(name, "w") 
...
#===============================================================================
### CHECK de que al archivo no le faltan ind de algun RUN
name = path +"/"+TOPIC +"_"+str(nRuns)+"_"+ objectives +"_"+ nGen+"_"+ popSize+ "_"+ indSize +"_" + cross +"_" + mut +"_precision_all_run.txt"
file_allPrecision = open(name, "r")   

# ver si alguna fila es vacia

for x in range(0,num_lines):
    columnas = len(opened_files)  #i.e nRuns      
    line = file_allPrecision.readline()
     
    if line != "\n" and line != "" : #linea vacia
        line = line.split()         
        if len(line)!=columnas:
            error_file = os.getcwd() + "/evolution_errors.txt"
            if os.path.exists(error_file):
                append_write = 'a' # append if already exists
            else:
                append_write = 'w' # make a new file if not
            print "NO MATCH IN NUMBER OF INDUVIDUALS, file: ",name           
            error_docs = open(error_file, append_write)
            error_docs.write("Fail in " + name)
            error_docs.write("\n")
            error_docs.close()  
            break 
...
file_all_height = open(path+"/" +TOPIC +"_"+str(nRuns)+"_"+ objectives +"_"+ nGen+"_"+ popSize+ "_"+ indSize +"_" + cross +"_" + mut +"_height_all_run.txt", "w")

opened_files = []
for F in height_files:
    opened_files.append(open(path+"/"+F, "r"))
...
for x in range(0,num_lines):
    for i in range(0, len(opened_files)):
        linea = str(opened_files[i].readline().strip())
        if linea != []:
            s_height.append(linea)
        else:
            s_height.append(" ")            

    table_h.append(s_height)
    s_height = []


file_all_height.write(tabulate(table_h,tablefmt="plain"))
file_all_height.close()
print "File created: *_height_all_run.txt"
